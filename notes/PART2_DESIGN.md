...
### 1.2 Frozen threshold: exact-histogram **Otsu 40049** (method frozen, per-scan value recorded)

The segmentation record in `data/9x9x9_octet_lattice/segmentation/report.md` (echoed in `candidate_otsu.json` and the run CSV/JSONs) computed two data-derived thresholds on this scan's exact uint16 histogram: **Otsu 40049** → foreground 58,675,274 (11.30 %), and **Triangle 34963** → foreground 100,399,672 (19.34 %). Part 1 *selected Triangle* for its segmentation deliverable because Otsu "lost visible struts."

- **Frozen rule for Part 2: `input >= 40049`** (exact-histogram Otsu) → **58,675,274 foreground voxels (11.30 %)** on this scan. This value has full repo provenance. The `40129` used in our earlier working notes has **no derivation anywhere in the segmentation record** — it was seeded into the `poc/ct_registration` brief; that POC did compute fg 58,324,474 at 40129 (`register_ct.py`), but the *number* 40129 itself is untraceable. The two thresholds are empirically near-identical (~4.95 % of the middle slice foreground either way), so no downstream conclusion changes — but 40049 is the value that is actually reproducible.
...
- **Freeze the *method*, record the *value*.** These uint16 intensities are uncalibrated reconstruction units (not Hounsfield), so the exact integer that means "titanium" drifts scan-to-scan with exposure/reconstruction even for identical Ti parts; only the Ti/air *contrast* is stable. The config therefore freezes the *recipe* (exact-histogram Otsu) and records **40049 / fg 58,675,274 as this scan's bit-exact replay target**, recomputed (not hardcoded) for any new specimen. As an independent sanity check, Stage 2 compares this ~11.3 % against the design's expected metal fraction (from `0.stl` solid volume, excluding the Y tabs/plates per §1.5) — corroboration, never a selection knob.
...
Registered-JSON positions are `[x, y, z]`; the numpy array is `(Z, Y, X) = (761, 815, 837)`. **Correct sampling is `vol[round(z), round(y), round(x)]`.** Validated empirically at the frozen threshold 40049: 94.5 % of 200 random registered junctions hit foreground within a 5×5×5 neighborhood (89.9 % at the exact voxel, median intensity 52,296). Because these baselines were measured at exactly the value Part 2 freezes (§1.2), Stage 2 reproduces them directly with no re-measurement discrepancy. Every plausible wrong mapping scores far worse (2.1–67.1 %), and `vol[x][y][z]` **goes out of bounds** (x max 773.7 > 761 pages). The briefing's "837×815×761" is X-by-Y-by-Z, i.e. reversed from numpy order. This mapping is pinned in the config and inside the corridor-sampling tool — no agent ever indexes the raw array from prose instructions.
...
flowchart TD
    ...
    STL --> S1
    TIF --> S2["Stage 2: data prep — config freeze, segmentation replay (Otsu 40049, bit-exact), registration QA gate"]
    RJ --> S2
...
| `data_prep` | 2 | Freezes `analysis/config/analysis_config.json` (provenance class per constant: verified / derived / external / open); **replay, not exploration** — runs the recorded segmentation recipe (exact-histogram Otsu) with the bit-for-bit foreground gate (58,675,274); runs registration QA on **all** edges; emits the x-binned bias curve; fixes notes hygiene; never touches ground-truth labels |
...
- **`ct-threshold-optimizer` (upgrade, off critical path):** add a frozen-threshold verify/replay mode (exact-histogram Otsu → 40049, expected fg 58,675,274); keep sweeping as demo fallback.
...
| 2 data prep (config + replay + registration QA) | **M2** — junction overlay on fresh mask | TIFF + registered JSON → `config/analysis_config.json`, `segmentation/mask.tif` (gitignored), `replay_report.json`, `slice_380.png`, `qa/registration_qa.json`, `qa/bias_by_x.png` | fg == 58,675,274 bit-for-bit (exact-histogram Otsu); junction 5×5×5 foreground ≥ 90 % (measured at 40049, matching §1.3's 94.5 %); HALT on fail |
...
| Risk | Mitigation |
|---|---|
| Threshold provenance conflict (`40129` has no repo provenance; `40049` is the genuine exact-histogram Otsu, fg 58,675,274) | Freeze the **Otsu method → 40049** in config with an explicit discrepancy note vs the earlier 40129; bit-exact per-scan replay gate; eval gate rejects any other hard-coded value |
| Axis-order silent failure (wrong mappings still return plausible 28–67 % foreground) | Mapping pinned in config **and inside the tools**; Stage 2 gate must reproduce the ~94.5 % 5×5×5 baseline (measured at 40049) before anything downstream runs |
